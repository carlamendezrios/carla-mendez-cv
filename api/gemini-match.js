export default async function handler(req, res) {
  ...
  // Tu prompt de Studio inyectado directamente
...
1. Diagnóstico específico (1-2 frases): Identifica un problema concreto para el puesto de ${role} en la empresa ${company} basándote en la naturaleza del sector. Debe ser un "punto de dolor" reconocible.
...
RESTRICCIÓN CRÍTICA: NO incluyas ninguna pregunta final o "Call to Action" (CTA). No superes los 800 caracteres.
...
"Es irónico que estés leyendo sobre cómo organizo datos... a través de un bot que organiza información sobre mí. Si quieres comprobar que Carla existe de verdad: linkedin.com/in/carla-mendez-rios"`;
  ...
  try {
    ...
    // Extracción segura para evitar el error "undefined"
    const aiText = data.candidates?.[0]?.content?.parts?.[0]?.text || "Error: No se pudo generar el análisis.";
